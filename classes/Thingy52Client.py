...
class Thingy52Client(BleakClient):
    ...
    async def receive_inertial_data(self, sampling_frequency: int = 30):
        """
    ...
    def raw_data_callback(self, sender, data):
        ...
        if len(self.data_buffer) == self.buffer_size:
            ...
            output = self.model.run(None, {input_: input_data})[0]
            cls_index = np.argmax(output, axis=1)[0]
            print(f"\r{self.mac_address} | {receive_time} - Prediction: {self.classes[cls_index]} | predict:{cls_index} | output:{output}", end="", flush=True)
            self.data_buffer.clear()

        self.data_buffer.append([acc_x, acc_y, acc_z, gyro_x, gyro_y, gyro_z])

        #(f"\r{self.mac_address} | {receive_time} - Accelerometer: X={acc_x: 2.3f}, Y={acc_y: 2.3f}, Z={acc_z: 2.3f}", end="", flush=True)
